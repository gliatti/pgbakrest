...
####################################################################################################################################
package pgBakRestTest::Common::ContainerTest;
...
sub sshSetup
{
    ...
    my $strScript = sectionHeader() .
        ...
        "    echo 'ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABAQDlXJKkRqflGftBDlc/XdiRfFOD5en0355zIOEP3Y5CTRVk4WcQqVdRoxcfRZWNFGCOBsXvwk" .
             ...
             "hLYkuCyAkdvbRInZcQjYiWf2yP6gJaAsnab5Eu9b user\@pgbakrest-test' > ${strUserPath}/.ssh/authorized_keys && \\\n" .
...
sub caSetup
{
    ...
    $strCertFile .= '/pgbakrest-test-ca.crt';
...
sub containerBuild
{
    ...
    foreach my $strOS ($strVm eq 'all' ? VM_LIST : ($strVm))
    {
        ...
        if (!$bDeprecated)
        {
            ...
            #-----------------------------------------------------------------------------------------------------------------------
            $strScript .= caSetup($strOS, $oStorageDocker, "test/certificate/pgbakrest-test-ca.crt");

            #-----------------------------------------------------------------------------------------------------------------------
            $strScript .= sectionHeader() .
                "# Create banner to make sure pgBakRest ignores it\n" .
                "    echo '***********************************************' >  /etc/issue.net && \\\n" .
