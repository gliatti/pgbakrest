...
####################################################################################################################################
package pgBakRestTest::Common::ExecuteTest;
...
use pgBakRestTest::Common::Io::Handle;
use pgBakRestTest::Common::Io::Buffered;
...
sub begin
{
    ...
    $self->{oIo} = new pgBakRestTest::Common::Io::Buffered(new pgBakRestTest::Common::Io::Handle('exec test', $self->{hOut}), 0, 65536);

    # Create buffered error object
    $self->{oIoError} = new pgBakRestTest::Common::Io::Buffered(
        new pgBakRestTest::Common::Io::Handle('exec test', $self->{hError}), 0, 65536);
...
sub executeTest
{
    ...
    my $oExec = new pgBakRestTest::Common::ExecuteTest($strCommand, $oParam);
    $oExec->begin();
    $oExec->end();
