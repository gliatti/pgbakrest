...
testRun(void)
{
    ...
    if (testBegin("zst"))
    {
        ...
        TEST_RESULT_Z(buffer, "{inputSame: true, inputOffset: 999, frameDone false, done: true}", "check log");
#else
        TEST_ERROR(compressTypePresent(compressTypeZst), OptionInvalidValueError, "pgBakRest not built with zst support");
#endif // HAVE_LIBZST
    }
    ...
    if (testBegin("helper"))
    {
        ...
        TEST_ERROR(compressTypePresent(compressTypeXz), OptionInvalidValueError, "pgBakRest not built with xz support");
