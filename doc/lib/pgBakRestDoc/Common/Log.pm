...
# COMMON LOG MODULE
####################################################################################################################################
package pgBakRestDoc::Common::Log;
...
sub logDebugProcess
{
    ...
    # Strip the package name off strFunction if it's pgBakRest
    $strFunction =~ s/^pgBakRest[^\:]*\:\://;
...
sub log
{
    ...
    if (defined($iCode))
    {
        return new pgBakRestDoc::Common::Exception($strLevel, $iCode, $strMessage, longmess(), $rExtra);
    }
