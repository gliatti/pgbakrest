...
# DOC EXECUTE MODULE
####################################################################################################################################
package pgBakRestDoc::Common::DocExecute;
use parent 'pgBakRestDoc::Common::DocRender';
...
use pgBakRestDoc::Common::Host;
use pgBakRestDoc::Common::HostGroup;
use pgBakRestDoc::Common::Ini;
...
sub backrestConfig
{
    ...
    if ($self->{bExe} && $self->isRequired($oSection))
    {
        ...
        else
        {
            ...
            my $strLocalFile = abs_path(dirname($0)) . '/output/pgbakrest.conf';
    ...
    if ($oChild->nameGet() eq 'host-add')
    {
        if ($self->{bExe} && $self->isRequired($oSection))
        {
            ...
            else
            {
                ...
                my $oHost = new pgBakRestDoc::Common::Host(
