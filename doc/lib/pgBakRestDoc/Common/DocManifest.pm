...
# DOC MANIFEST MODULE
####################################################################################################################################
package pgBakRestDoc::Common::DocManifest;
...
sub new
{
    ...
    # Load the manifest
    $self->{oManifestXml} = new pgBakRestDoc::Common::Doc("$self->{strDocPath}/manifest.xml");
    ...
    foreach my $oSource ($self->{oManifestXml}->nodeGet('source-list')->nodeList('source'))
    {
        ...
        if (defined($strFile))
        {
            $oSourceHash->{doc} = new pgBakRestDoc::Common::Doc($self->{strDocPath} . "/${strFile}");
        }
        # Else should be in doc/xml
        else
        {
            $$oSourceHash{doc} = new pgBakRestDoc::Common::Doc("$self->{strDocPath}/xml/${strKey}.xml");
        }
...
sub isBackRest
{
    ...
    return($self->variableTest('project-exe', 'pgbakrest'));
}
