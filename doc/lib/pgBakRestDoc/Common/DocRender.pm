...
# DOC RENDER MODULE
####################################################################################################################################
package pgBakRestDoc::Common::DocRender;
...
sub new
{
    ...
    if (defined($self->{strRenderOutKey}))
    {
        ...
        {
            require pgBakRestDoc::Custom::DocCustomRelease;
            pgBakRestDoc::Custom::DocCustomRelease->import();

            $self->{oDoc} =
                (new pgBakRestDoc::Custom::DocCustomRelease(
