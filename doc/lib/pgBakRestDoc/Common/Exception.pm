...
####################################################################################################################################
package pgBakRestDoc::Common::Exception;
...
sub isException
{
    ...
    if (defined($roException) && defined($$roException))
    {
        ...
        {
            return $$roException->isa('pgBakRestDoc::Common::Exception') ? 1 : 0;
        }
        ...
        elsif ($$roException =~ /^PGBRCLIB\:[0-9]+\:/)
        {
            ...
            # Create exception
            $$roException = new pgBakRestDoc::Common::Exception("ERROR", $iCode, $strMessage, $strTrace, undef, 1);
