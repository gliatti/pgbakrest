...
####################################################################################################################################
package pgBakRestDoc::Html::DocHtmlPage;
use parent 'pgBakRestDoc::Common::DocExecute';
...
use pgBakRestDoc::Common::DocRender;
use pgBakRestDoc::Html::DocHtmlBuilder;
use pgBakRestDoc::Html::DocHtmlElement;
...
sub process
{
    ...
    my $oHtmlBuilder = new pgBakRestDoc::Html::DocHtmlBuilder(
...
sub sectionProcess
{
    ...
    my $oSectionTocElement = new pgBakRestDoc::Html::DocHtmlElement(HTML_DIV, "section${iDepth}-toc");

    # Create the section element
    my $oSectionElement = new pgBakRestDoc::Html::DocHtmlElement(HTML_DIV, "section${iDepth}");
...
sub backrestConfigProcess
{
    ...
    if ($bShow)
    {
        ...
        # Render the config
        $oConfigElement = new pgBakRestDoc::Html::DocHtmlElement(HTML_DIV, "config");
...
sub postgresConfigProcess
{
    ...
    if ($bShow)
    {
        ...
        $oConfigBodyElement->
            addNew(HTML_DIV, "config-body-output",
                   {strContent => defined($strConfig) ? $strConfig : '<No PgBakRest Settings>'});
