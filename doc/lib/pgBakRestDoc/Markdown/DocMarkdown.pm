...
# DOC MARKDOWN MODULE
####################################################################################################################################
package pgBakRestDoc::Markdown::DocMarkdown;
...
use pgBakRestDoc::Markdown::DocMarkdownRender;
...
sub process
{
    ...
    foreach my $strRenderOutId ($self->{oManifest}->renderOutList(RENDER_TYPE_MARKDOWN))
    {
        ...
        $self->{oManifest}->storage()->put(
            $strFile, $self->{oManifest}->variableReplace((new pgBakRestDoc::Markdown::DocMarkdownRender($self->{oManifest},
