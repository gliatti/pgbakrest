...
# DOC RELEASE MODULE
####################################################################################################################################
package pgBakRestDoc::Custom::DocCustomRelease;
...
sub docGet
{
    ...
    my $oStorageDoc = new pgBakRestTest::Common::Storage(
        dirname(abs_path($0)), new pgBakRestTest::Common::StoragePosix({bFileSync => false, bPathSync => false}));
    my @hyGitLog = @{(JSON::PP->new()->allow_nonref())->decode(${$oStorageDoc->get("resource/git-history.cache")})};

    # Get renderer
    my $oRender = new pgBakRestDoc::Common::DocRender('text');
    $oRender->tagSet('backrest', PROJECT_NAME);

    # Create the doc
    my $oDoc = new pgBakRestDoc::Common::Doc();
