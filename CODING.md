# pgBakRest <br/> Coding Standards

## Formatting with uncrustify

pgBakRest uses uncrustify to check/update the code formatting. If the `code-format` test fails in CI then reformat the code:
```
pgbakrest/test/test.pl --code-format
```
...
```c
storagePathCreateP(storageLocal(), "/tmp/pgbakrest");
```
Or with variable parameters:
```c
storagePathCreateP(storageLocal(), "/tmp/pgbakrest", .errorOnExists = true, .mode = 0777);
```
